import Reveal from '../reusables/Reveal'

function MadrasaVision() {
  return (
    <section className="bg-white py-18 sm:py-20">
      <div className="mx-auto max-w-4xl px-6 text-center">
        <Reveal>
          <p className="text-sm font-bold uppercase tracking-[0.2em] text-[#2f6a52]">
            A New Model of Madrasa Education
          </p>
          <h2 className="mt-5 font-serif text-[2.4rem] leading-[1.02] tracking-[-0.04em] text-[#1f382a] sm:text-[3.2rem]">
            Rooted in deen, prepared for the modern world.
          </h2>
          <p className="mx-auto mt-7 max-w-[46rem] text-[1.02rem] leading-[1.9] text-[#5f6d64]">
            This madrasa was established to move beyond the conventional system
            and introduce a modern, time-appropriate curriculum that integrates
            Arabic and Bengali education.
          </p>
          <p className="mx-auto mt-5 max-w-[46rem] text-[1.02rem] leading-[1.9] text-[#5f6d64]">
            The goal is not only to produce students with strong religious
            foundations, but also to equip them with essential academic
            knowledge needed for the modern world.
          </p>
        </Reveal>
      </div>
    </section>
  )
}

export default MadrasaVision
