...
import { getFeaturedMadrasa } from './data'
import { usePublicStats } from '../../lib/publicStats'

function FeaturedMadrasa() {
  const { t } = useTranslation()
  usePublicStats()
  const featuredProgram = getFeaturedMadrasa(t)

  return (
    <section className="bg-[#fbfdfb] py-20 sm:py-24">
      <div className="mx-auto grid max-w-7xl gap-12 px-6 lg:grid-cols-[1.08fr_0.92fr] lg:items-center lg:gap-18">
        <Reveal className="max-w-2xl" delay={120}>
          ...
          <div className="mt-8 grid gap-5 sm:grid-cols-3">
            {featuredProgram.stats.map((stat) => (
              <div
                className="rounded-[1rem] border border-[#dbe7ee] bg-white px-5 py-5"
                key={stat.label}
              >
...
export default FeaturedMadrasa
